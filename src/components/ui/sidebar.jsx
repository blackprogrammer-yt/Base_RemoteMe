...
const SidebarProvider = React.forwardRef(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
    ...
    const setSidebarWidthSafe = React.useCallback((value) => {
...
const SidebarRail = React.forwardRef(({ className, ...props }, ref) => {
    const { toggleSidebar, setWidth, isMobile } = useSidebar();
    const handlePointerDown = (event) => {
        ...
        const handlePointerMove = (moveEvent) => {
            ...
            const minPx = SIDEBAR_MIN_WIDTH_REM * 16;
            const maxPx = SIDEBAR_MAX_WIDTH_REM * 16;
            const raw = moveEvent.clientX;
            const clamped = Math.min(Math.max(raw, minPx), maxPx);
            const widthRem = clamped / 16;
            setWidth(`${widthRem}rem`);
        };
